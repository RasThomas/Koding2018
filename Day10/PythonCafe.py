...
class PythonCafe:
    ...
    def home(self, action):
        ...
        if action == 'Get Products':
            stringTest = ""
            test = readTableItems(DB_NAME)
            for i in range(0, len(test)):
                stringTest = stringTest + str(test[i]) + "</br>"
            return stringTest;
    ...
    @expose
    def makeNewCustomer(self, inputName, email, phone, username, password, paymentMode, address, membershipId):
        # read the posted values from the UI
        sql = 'INSERT INTO Customer (Name, Email, Phone, UserName, Password, PaymentMode, Address, MembershipId) VALUES (?, ?, ?, ?, ?, ?,? ,? )'
        item = (inputName, email, phone, username, password, paymentMode, address, membershipId)
        print(item)
        insertDataIntoTable(DB_NAME, item, sql)
        return open('NewCustomer.html')
